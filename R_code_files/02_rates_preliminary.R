...
#Read in OECD dataset for non-OECD Countries for the year 2022

non_oecd_data_2022 <- get_dataset("CTS_CIT", start_time = 2022, end_time = 2022)

#Keep and rename selected columns

non_oecd_data_2022 <- subset(non_oecd_data_2022, non_oecd_data_2022$CORP_TAX=="COMB_CIT_RATE")
non_oecd_data_2022 <- subset(non_oecd_data_2022, select = -c(CORP_TAX,TIME_FORMAT,Time))

colnames(non_oecd_data_2022)[colnames(non_oecd_data_2022)=="ObsValue"] <- "2022"
colnames(non_oecd_data_2022)[colnames(non_oecd_data_2022)=="COU"] <- "iso_3"

#Combine 2022 data ("oecd_2022") with 2022 non_OECD data
oecd_all_2022 <- merge(oecd_data_2022, non_oecd_data_2022, by="iso_3", all=T)

#Keep and rename selected columns
colnames(oecd_all_2022)[colnames(oecd_all_2022)=="2022.x"] <- "redundant"
colnames(oecd_all_2022)[colnames(oecd_all_2022)=="2022.y"] <- "2022"

oecd_all_2022 <- subset(oecd_all_2022, select = -c(redundant))


#Combine 2022 data ("oecd_all_2022") with data from previous years ("previous_years")
all_years_preliminary <- merge(oecd_all_2022, previous_years, by="iso_3", all=T)
all_years_preliminary <- all_years_preliminary[, c("iso_2", "iso_3", "continent", "country", 1980, 1981, 1982, 1983, 1984, 1985, 1986, 1987, 1988, 1989, 1990, 1991, 1992, 1993, 1994, 1995, 1996, 1997, 1998, 1999, 2000, 2001, 2002, 2003, 2004, 2005, 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022)]
